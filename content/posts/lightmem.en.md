...
Compression starts by running the input through LLMLingua-2, token by token. Each token gets a retain probability, a score for how worth keeping it is, and any token scoring below a threshold ($\tau$) gets dropped without exception. The criterion is information content: tokens with high entropy, the hard-to-predict, information-dense ones, get kept first.

![](/images/lightmem/image2.png)

This filtering step removes 50 to 70 percent of all tokens, and semantic coherence survives intact. What matters isn't token count. It's how much information those tokens actually carry.

## Topic Segmentation: Splitting Conversation by Meaning

Once compressed sensory memory gets batched by turn count, topic segmentation kicks in: detecting where a conversation or document shifts subject and splitting the data there. LightMem combines two signals for this, attention and embedding similarity.

The first signal is the attention-based boundary candidate ($B_1$). The LLM computes an attention score for each conversational turn, and any turn where that score hits a local maximum gets flagged as a topic shift.

![](/images/lightmem/image3.png)

The second signal is the embedding-similarity boundary candidate ($B_2$). When the embedding similarity between adjacent turns drops below a threshold ($\tau$), that point also gets flagged as a shift.
...
Each signal covers what the other might miss on its own. Unlike splitting at fixed intervals, boundaries decided this way capture the specific context each document or conversation actually has.

## Topic-Aware Short-Term Memory

Topic segmentation produces memory fragments split by topic. These get pushed into an STM buffer as $\{topic, \{user_i, model_i\}\}$ pairs, where $user_i$ is the user's input and $model_i$ is the model's response.

Once the buffer hits its size threshold, an LLM call summarizes its contents. That summary gets stored alongside the original exchanges in long-term memory as $\{topic, \{sum_i, user_i, model_i\}\}$.
...
LightMem was evaluated under Incremental Dialogue Turn Feeding, an environment that mimics how turns actually arrive in a live conversation, one at a time. The pre-compressor was LLMLingua-2, running on a lightweight BERT architecture, with the sensory memory buffer set to 512. Evaluation ran on LongMemEval and LoCoMo, against Full Text, Naive RAG, LangMem, A-MEM, MemoryOS, and Mem0 as baselines.

![](/images/lightmem/image6.png)

Under identical model conditions, LightMem outperformed every comparison system while using a fraction, in some cases just a few percent, of their token budget. Latency came out dramatically ahead as well.

### How the Retention Ratio Affects Performance

The experiments also varied the retention ratio ($r$), which controls how many tokens survive pre-compression. A higher $r$ lowers the threshold ($\tau$), letting more tokens through. Across the board, $r = 0.6$ generally performed best, though the ideal value shifted with STM buffer size: smaller buffers favored 0.6, while larger buffers did better at 0.7.
