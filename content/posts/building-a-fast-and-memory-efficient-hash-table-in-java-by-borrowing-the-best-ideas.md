...
The twist is that SwissTable separates *metadata* (tiny "control bytes") from the actual key/value storage, and it uses those control bytes to avoid expensive key comparisons most of the time. Instead of immediately touching a bunch of keys (which are cold in cache and often pointer-heavy), it first scans a compact array of control bytes that is dense, cache-friendly, and easy to compare in bulk. If this feels like a tiny Bloom filter, you're on the right track: the control bytes act as a cheap "maybe" gate before we pay for real key comparisons.
...
The following pieces survived the usual round of "this feels clever but isn't fast" refactors.
...
In Java, the cost is higher: "keys/values" usually means arrays of references. On 64-bit JVMs, **compressed oops** (often enabled up to ~32GB depending on alignment/JVM flags) packs references into 32 bits, making the reference arrays denser. When compressed oops is off, references widen to 64 bits and the same number of key touches can spill across more cache lines.

Either way, the ctrl array does most of the work: most misses die in metadata. Compressed oops makes the unavoidable key touches cheaper when they do happen.
...
On `put`, tombstones serve two purposes: they preserve correctness, and they're reusable space. The common pattern is:
...
That tends to keep probe chains from getting longer over time, reduce resize pressure, and prevent workloads with lots of remove/put cycles from slowly poisoning performance.
...
If you want the "straight from the source" version, Matt Kulukundis and other Google engineers' CppCon talk on SwissTable is excellent, clear, practical, and packed with the kind of details that make the design click: [CppCon talk](https://www.youtube.com/watch?v=ncHmEUmJZf4).